...
# for iks
onready var backboard = $CanvasLayer/FloorBoard/BackBoard
onready var backfoot = $CanvasLayer/FloorBoard/BackFoot
onready var frontfoot = $CanvasLayer/FloorBoard/FrontFoot
onready var frontboard = $CanvasLayer/FloorBoard/FrontBoard
...
func _ready():
	...
	rig.connect("dragon_anim_event", self, "_anim_event")
	
	$CanvasLayer/FloorBoard.get_armature().get_slot('player').set_display_index(-1)
...
func _anim_event(details : Dictionary):
...
#	rig.get_armature().set_ik_constraint("right_arm_ik", mouse_offset);    
#
#	rig.get_armature().get_bone("github_bone").set_bone_position(spawn_pos)
#	rig.get_armature().get_bone("github_bone").set_bone_scale(spawn_pos / 100)

	var floorboard = $CanvasLayer/FloorBoard
	var board_root_position = floorboard.get_armature().get_bone("floor_board").get_bone_position()
	rig.get_armature().get_bone('root').set_bone_position(board_root_position)
	rig.get_armature().set_ik_constraint('bone_ik', (backboard.global_position - rig.global_position))
	rig.get_armature().set_ik_constraint('bone_ik1', (frontboard.global_position - rig.global_position))
	rig.get_armature().set_ik_constraint('calf_l', (frontfoot.global_position - rig.global_position))
	rig.get_armature().set_ik_constraint('calf_r', (backfoot.global_position - rig.global_position))
	
	
	if Input.is_action_pressed("ui_left"):
		$CanvasLayer/FloorBoard.rotation -= step
	if Input.is_action_pressed("ui_right"):
		$CanvasLayer/FloorBoard.rotation += step
		
	if Input.is_action_pressed("ui_up"):
		var current_rotation = rig.get_armature().get_bone("pelvis").get_bone_rotation()
		rig.get_armature().get_bone("pelvis").set_bone_rotation(current_rotation - step)
	if Input.is_action_pressed("ui_down"):
		var current_rotation = rig.get_armature().get_bone("pelvis").get_bone_rotation()
		rig.get_armature().get_bone("pelvis").set_bone_rotation(current_rotation + step)
		
	print($CanvasLayer/FloorBoard.rotation)
